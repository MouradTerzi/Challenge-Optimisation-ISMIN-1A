    ...
    for (int i = 0; i < sol.pDat->getNodes(); i++)
        for (int j = 0; j < sol.pDat->getNodes(); j++)
            if (sol.tree[i][j])
                flot << "(" << i << "," << j << ") ";
    flot << endl << endl;
    flot << " Co�t total : " << sol.cost << endl<<endl;
    return flot;
}
...
bool Solution::feasible() const
{
    ...
    for (auto it = route.begin(); it != route.end(); it++)
    {
        ...
        {
            cout << "Erreur client "<<*it<<" visité plusieurs fois dans la route " << endl;
            feas = false;
        }
        present[*it] = true;
    } 
    ...
    //v�rification doublons dans tree et comptage du nombre d'arcs
    for (int i = 0; i < pDat->getNodes(); i++)
        for (int j = 0; j < pDat->getNodes(); j++)
            if (tree[i][j] == true)
            {
    ...
    // v�rification connexite tree et profondeur
    ...
    for (int i = 0; i < pDat->getNodes(); i++) //connexit�
    ...
    // v�rification que depot du v�hicule n'est pas dans arbre
